module r_lambda
    ...
    real(8), save :: l, lin, k12, k21, over_2k12, over_2k21, &
                     over_nosc1_minus1, over_nosc2_minus1, pi, pi2
...
program main
    ...
    nosc = nosc1 + nosc2
    nosc1_plus1 = nosc1 + 1
    over_nosc1_minus1 = 1.0 / (nosc1-1)
    over_nosc2_minus1 = 1.0 / (nosc2-1)

    allocate(omega(nosc), y(nosc), yp(nosc), A(nosc1,nosc2))
...
subroutine F(t,y,yp)
    ...
    do i=1,nosc1
        ...
        soma = sum( sin(y(1:nosc1) - y(i)) )

        yp(i) = yp(i) + lin * soma * over_nosc1_minus1

        soma = sum( sin(y(nosc1_plus1:nosc) - y(i)), mask = A(i,:) == 1.0_rk )
        
        yp(i) = yp(i) + l * soma * over_2k12

    end do

    do i=nosc1_plus1,nosc

        soma = sum( sin(y(1:nosc1) - y(i)), mask = A(:,i-nosc1) == 1.0_rk )

        yp(i) = yp(i) + l * soma * over_2k21

        soma = sum( sin(y(nosc1_plus1:nosc) - y(i)) )
        
        yp(i) = yp(i) + lin * soma * over_nosc2_minus1
